...
def load_dataset(dataset_directory):
    ...
    for split in ["train", "dev", "test"]:
        if split == "test":
            split_jsonl_file = os.path.join(
                dataset_directory,
                f"CT23_1A_checkworthy_multimodal_english_dev_{split}.jsonl",
            )
        else:
            split_jsonl_file = os.path.join(
                dataset_directory,
...
def load_data_splits_with_gold_dataset(dataset_directory, version, **kwargs):
    ...
    for split in splits:
        if split == "gold":
            ...
            split_jsonl_file = (
                f"{data_dir}/CT23_1A_checkworthy_multimodal_english_{split_name}.jsonl"
            )
        with open(split_jsonl_file, "r") as f:
...
def get_samples_with_excess_tokens(
    token_limit, split_to_tokenized_texts, padding_token
):
    ...
    for split in SPLITS:
        for idx, item in enumerate(split_to_tokenized_texts[split]):
            ...
            if seq_length > token_limit:
                split_to_examples_with_excess_tokens[split].append(
                    (idx, seq_length, seq_length - token_limit)
                )  # record how many tokens would be truncated from which sample
...
def further_normalize_samples_with_excess_tokens(
    token_limit,
    split_to_normalized_texts,
    split_to_samples_with_excess_tokens,
    split_to_tokenized,
    tokenizer,
    padding_token,
):
    ...
    for split, excess_examples in split_to_samples_with_excess_tokens.items():
        for excess_example in excess_examples:
            ...
            further_norm_tokens = tokenizer(
                further_norm_text, padding=True, return_tensors="pt"
            )["input_ids"][0]

            # New sequence length
            seq_length = (
                (further_norm_tokens != padding_token)
            ...
            if seq_length > token_limit:
                ...
                final_split_to_normalized_texts[split][idx] = " ".join(
                    further_norm_text.split()[:-num_excess_tokens]
                )
            else:
...
def embed_and_pickle_split(
    ...
    ocr_method_string = (
        f"{model_name}_with_ocr_{split_name}_{dataset_version}"
        if with_ocr
        else f"{model_name}_{split_name}_{dataset_version}"
    )
...
def embed_and_pickle_split_with_bertweet(
    bertweet_model,
    pickle_dir,
    split_name,
    encoded_split,
    ...
    # Gather information about the split
    ocr_method_string = (
        f"bertweet_embeddings_with_ocr_{split_name}_{dataset_version}"
        if with_ocr
        else f"bertweet_embeddings_{split_name}_{dataset_version}"
    )
    pickle_file = f"{pickle_dir}/{ocr_method_string}.pickle"
...
def pickle_all_splits(
    ...
    for split, features in split_to_features.items():
        ...
        if split == GOLD:
            pickle_file = (
                f"{directory}/features/{feature_method}/{feature_method}_{split}.pickle"
            )
        else:
        ...
        if reload_and_check:
            print(
                f"Pickled and initial feature matrix same? "
